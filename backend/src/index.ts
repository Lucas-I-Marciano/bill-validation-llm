...
if (!GEMINI_API_KEY) {
    console.error("Erro: Chave da API do Gemini (GEMINI_API_KEY) não configurada no .env");
    process.exit(1);
}
...
async function checkDuplicateReading(customerCode: string, measureType: 'WATER' | 'GAS', measureDate: Date): Promise<boolean> {
...
const UploadRequestBodySchema = z.object({
    ...
    measure_datetime: z.string().datetime({ message: "Data/hora da medição inválida" }), // Valida formato ISO 8601
    measure_type: MeasureTypeEnum,
});

// Helper para formatar erros Zod
function formatZodError(error: ZodError): { error_code: string, error_description: string } {
    const descriptions = error.errors.map(err => `${err.path.join('.')}: ${err.message}`).join('; ');
    return {
...
function extractBase64Data(base64String: string): { mimeType: string, data: string } {
    ...
    console.warn("Prefixo MIME Type não encontrado no base64, assumindo image/jpeg");
...
app.post('/upload', async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const { image: base64Input, customer_code, measure_datetime, measure_type } = validationResult.data;
        const measureDate = new Date(measure_datetime);

        // 2. Verificar Duplicidade
        const isDuplicate = await checkDuplicateReading(customer_code, measure_type, measureDate);
        if (isDuplicate) {
        ...
        const prompt = `Analise a imagem do medidor (${measure_type === 'WATER' ? 'água' : 'gás'}) e extraia APENAS o valor numérico inteiro da leitura principal exibida. Retorne somente o número.`;
        ...
        // 4. Processar Resposta do Gemini
        ...
        if (isNaN(measureValue)) {
            ...
            return res.status(500).json({ // Ou talvez 400 se considerar falha na imagem/prompt?
                error_code: "LLM_EXTRACTION_FAILED",
                error_description": "Não foi possível extrair um valor numérico da imagem."
            });
        }
        ...
        // 6. Salvar Imagem Temporariamente e Gerar URL
        const imageBuffer = Buffer.from(base64Data, 'base64');
        const imageFileName = `${measure_uuid}.${mimeType.split('/')[1] || 'jpg'}`; // Usa UUID como nome + extensão
        const imagePath = path.join(UPLOAD_DIR, imageFileName);

        await fs.promises.writeFile(imagePath, imageBuffer);
        console.log(`Imagem salva temporariamente em: ${imagePath}`);

        // Gera a URL pública (assumindo que o servidor está rodando na raiz '/')
        // Em produção, seria uma URL de um CDN ou Storage Bucket
        const image_url = `${req.protocol}://${req.get('host')}/uploads/${imageFileName}`;

        // 7. (Simulado) Salvar no Banco de Dados
        const year = measureDate.getFullYear();
        const month = (measureDate.getMonth() + 1).toString().padStart(2, '0');
        addReadingToDB({
        ...
        console.error("Erro inesperado no endpoint /upload:", error);
        // Passa para o middleware de erro genérico
        next(error);
    }
});
